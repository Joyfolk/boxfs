...
public class BoxSeekableByteChannel implements SeekableByteChannel {
    ...
    BoxSeekableByteChannel(BoxFileSystem fileSystem, BoxPath path, Set<? extends OpenOption> options)
        ...
        this.path = (BoxPath) path.toAbsolutePath();
        this.position = 0;
        this.open = true;
        ...
        // If WRITE or APPEND, turn on write mode
        if (write || append || truncateExisting) {
            write = true;
        }

        // Can't have both READ and APPEND
        if (append && read && !write) {
            read = false;
        }

        this.readable = read;
        this.writable = write || append;

        // Get or create the file
        var existingInode = fileSystem.resolvePathToInode(this.path);
        ...
        if (existingInode.isPresent()) {
            ...
            this.inode = fileSystem.createFile(this.path);
        }
